...
userRouter.post("/register", async (req, res) => {
  const {firstname, fullname, email, password, phone } = req.body;

  const hashedPassword = await hashPassword(password);
  const userUsed = await getUserByEmail(req.body.email);

  if (userUsed == null) {
      createUser({firstname, fullname, email, password: hashedPassword, phone});
      return res.status(200).send("Create user succesfully");
  }else 
